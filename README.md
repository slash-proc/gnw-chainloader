...
It resides at `0x08000000` (Bank 1) and uses a **Dieted RAM-boot architecture**: a minimal bare-metal loader (flash stub) inflates the main application from an LZMA-compressed blob into AXI-SRAM. This maximizes space, allowing for rich features like exFAT support within the strict 32 KiB internal flash limit. It evaluates boot conditions on startup and headlessly jumps to either the patched Official Firmware (OFW) on Bank 2 or Retro-Go Launcher on Bank 1.
...
*   **Premium UI Themes:** 'Frosted Glass' (Mario) and 'Gilded Forest' (Zelda) themes featuring semi-transparent blending and persistent background animations.
...
*   **Protected Warm Resets:** Deduces return intent from Retro-Go magic words and utilizes a 4KB DTCM Safe Zone to preserve game state across reboots.
*   **Toggle Fast-Boot Setting:** Configure the device via the Tools menu to bypass the chainloader and boot straight to Retro-Go on startup, with a dedicated hardware override (holding **PAUSE/SET** at boot) to return to the launcher.
...
Flashing configurations automatically enforce target binary sizes (under **32,767 bytes**) and clean up any dynamically generated submodule cache files.
