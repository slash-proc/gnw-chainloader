...
* **OpenOCD Error Handling:** Prompt the user immediately if you see the error: `Unable to parse read_uint32 response: ""` during OpenOCD operations. **Workaround:** If this happens, run `python3 scripts/debug/trace.py reset-halt` to force a hardware reset of the hung MCU, which restores the OpenOCD connection and allows subsequent flash or read commands to succeed.
